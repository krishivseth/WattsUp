...
function App() {
  ...
  const getSafetyAnalysis = async (address: string): Promise<SafetyAnalysis | null> => {
    const API_ENDPOINT = "http://127.0.0.1:5002/api/safety";
  ...
  const calculateEnergyCost = async () => {
    try {
      ...
      // Get both energy cost and safety data concurrently
      const [energyCost, safetyAnalysis] = await Promise.all([
        getEnergyCost(currentAddress),
        getSafetyAnalysis(currentAddress)
      ]);

      setCost(energyCost);
      if (safetyAnalysis) {
        setSafetyData(safetyAnalysis);
      }
    } catch (err) {
  ...
  const handleRouteRequest = async (destination: string) => {
    try {
      ...
      const url = chrome.runtime.getURL(`route-planner.html?${params.toString()}`);
        ...
        <div className="border-t border-gray-200 pt-3 mt-3">
          ...
          {safetyData ? (
            <div className="space-y-3">
              ...
              />
              <SafetyDetails
                summary={safetyData.safety_summary}
